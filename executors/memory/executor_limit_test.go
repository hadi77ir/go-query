...
func TestMemoryExecutor_LimitEnforcement(t *testing.T) {
	data := getTestData() // 10 products
	...
	t.Run("limit with descending sort", func(t *testing.T) {
		...
		assert.Equal(t, 10, products[0].ID)
		assert.Equal(t, 9, products[1].ID)
		assert.Equal(t, 8, products[2].ID)
	})
...
func TestMemoryExecutor_LimitWithLargeDataset(t *testing.T) {
	data := generateLargeDataset() // 1000 products
	opts := query.DefaultExecutorOptions()
	opts.DefaultSortField = "id"
	...
	t.Run("limit exactly one page", func(t *testing.T) {
		p, err := parser.NewParser("limit = 50 page_size = 50 sort_by = id")
		require.NoError(t, err)
		q, err := p.Parse()
		require.NoError(t, err)
...
func TestMemoryExecutor_LimitEdgeCases(t *testing.T) {
	data := getTestData()
	opts := query.DefaultExecutorOptions()
	opts.DefaultSortField = "id"
	executor := NewExecutor(data, opts)
	...
	t.Run("limit with quoted string (consistency with page_size)", func(t *testing.T) {
		// Test that quoted limit values work, just like page_size
		p, err := parser.NewParser(`limit = "3" page_size = "5" sort_by = id`)
		require.NoError(t, err)
		q, err := p.Parse()
		require.NoError(t, err)
