...
func generateLargeDataset() []Product {
...
// TestMemoryExecutor_CursorPaginationLargeDataset tests cursor pagination with 1000 entries
func TestMemoryExecutor_CursorPaginationLargeDataset(t *testing.T) {
	...
	t.Run("backward pagination", func(t *testing.T) {
		...
		assert.Equal(t, page2[0].ID, backToPage2[0].ID)
		assert.Equal(t, page2[len(page2)-1].ID, backToPage2[len(backToPage2)-1].ID)
	...
	t.Run("edge cases - first and last page", func(t *testing.T) {
		...
		assert.Equal(t, 100, resultFirst.ShowingTo)
		...
		for {
			...
			if result.NextPageCursor == "" {
				...
				assert.NotEmpty(t, result.PrevPageCursor, "Last page should have prev cursor")
